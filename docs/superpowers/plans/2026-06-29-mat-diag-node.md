...
> **Status: ✅ COMPLETE（main にマージ済み）.** 全タスク実装・実機 E2E 検証済み（jarvis node 5）。後続の実機知見（`advertised_self_fabric` の CFID 取得）は Issue #3 で別途対応・クローズ済み。
...
- [x] **Step 1: Write the module with types and a smoke test**
...
- [x] **Step 1: Write the failing tests (all branches)**
...
- [x] **Step 4: Implement `node()` default path**
...
- [x] **Step 1: Create fake probe fixtures**
...
- [x] **Step 2: Run the full CI-equivalent check**
