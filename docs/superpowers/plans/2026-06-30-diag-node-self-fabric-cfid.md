...
> **Status: ✅ COMPLETE（main にマージ済み、Issue #3 クローズ）.** 全タスク実装・実機 E2E 検証済み（jarvis node 5 で `advertised_self_fabric:true`）。
>
> **実装時の乖離（重要）:** Task 5 の実機実測で**真因が判明し計画から変わった**。issue/spec が疑った「chip-tool のログ verbosity 不足」は誤りで、真因は **chip-tool が CFID シグナル（`[FP]` / `[DIS]`）を stdout に出すのに `mat` は `op_out.stderr` だけを見ていた**というストリーム取り違え（既定 verbosity で CFID は出ている）。このため計画外の是正コミット `c08bbf3`（**stdout+stderr 結合パース**）を追加し、fake に `FAKE_CHIP_CFID_STDOUT` と回帰テストを足した。Task 5 Step 3（verbosity 対応）は不要のためスキップ（YAGNI）。以下のチェックはこの経緯を踏まえた完了印。詳細はメモリ `chip-tool-logs-to-stdout` とコミット `c08bbf3`。
...
- [x] **Step 3: fake-chip-tool に `[DIS]` 行と抑止スイッチを実装**
...
- [x] **Step 1: 実機で operational read の生 stderr を採取**
...
- [x] **Step 2: fake の `[DIS]` 行を実機の形に合わせる（必要時）**
...
- [x] **Step 3: 既定で CFID が一切出ない場合のみ verbosity 対応（条件付き）**
...
- [x] **Step 5: コミット（Step 2/3 で変更があった場合のみ）**
...
- [x] **Step 1: README に `advertised_self_fabric` / `cfid_unavailable` を説明**
