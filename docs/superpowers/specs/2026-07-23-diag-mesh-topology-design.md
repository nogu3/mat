...
- **案 B の実態**は未知参加者の同定・マージ。fabric ノードで 0x33 read が
  失敗した場合は自己同定不能のまま出力する。ただしそのノードのテーブル行
  自体は有効な観測なので、`node:<node_id>` を自視点の頂点として自視点
  エッジ（実装は他ノードとの突合を要さない自ノード → 相手 `ext:` の 1 本）
  を張る（2026-07-23 実機 E2E での訂正: 以前は「他ノード視点のみ成立」
  だった）。
- 実機で同一 ext（HardwareAddress）を 2 台以上の fabric ノードが自己同定
  した場合（NL68 系 FW バグ、下記「実機 E2E 知見」参照）は、物理的に
  あり得ない衝突として該当ノード全員の自己同定を無効化する
  （＝それぞれ `node:<node_id>` へフォールバックする）。
- neighbor/route-table 行の `ExtAddress` が 0 の行はゴミ行として無視する
  （ノード化もエッジ化もしない）。
...
- ノードの安定キー `id` は `ext:<HEX16>`、ExtAddress 不明なら `rloc:<hex>`
  ...
  到達しない）、どちらも無い fabric ノード（0x33 が読めず probe 失敗等、
  または重複 self-ext 申告で無効化された場合）は `node:<node_id>`。
  自己同定不能な fabric ノードも `node:<node_id>` を頂点として自視点の
  エッジを持てる（上記「集約は純ロジック」節参照）。その電波実体が
  別ノードの neighbor/route-table 観測から `ext:` unknown ノードとして
  別途現れることがあり、`mat` は両者を同一エンティティへ相関・マージ
  しない（将来課題）。
...
- `mesh-local-prefix` の hex 表現は実機で 2 通り観測される: 8B 素形
  （16 桁 hex）と長さ前置形（0x40 = 64bit 長バイト + 8B prefix = 18 桁
  hex、NL68 系）。両方を正規化して 16 桁 hex（小文字）へ落とす
  （`mat-core::mesh::canon_ml_prefix`）。それ以外の桁数・非 hex は不正形
  として無視する。
...
### 実機 E2E 知見 (2026-07-23)

`mat diag mesh` ブランチの実機 E2E で以下 3 件を観測し、`mat-core::mesh` に
反映した:

- 一部の ESP32 系デバイス（tape light 群）が cluster 0x33 で工場出荷時の
  ハードコード HardwareAddress を**全台共通**で申告し、かつ IPv6Addresses
  が空だった。fabric 7 ノードが同一 ext を申告した結果、単一のグラフ頂点へ
  マージされ id 衝突・17 本のエッジが混線した。対応: 同一 ext を 2 台以上が
  自己同定した場合は全員の自己同定を無効化し `node:<node_id>` へフォール
  バックする（そのノード自身のテーブル観測は引き続き `node:<node_id>` を
  頂点として自視点エッジに使う）。
- 一部デバイスが `mesh-local-prefix` を長さ前置 octstr（`0x40` = 64bit 長
  バイト + 8B prefix = 18 桁 hex）で返し、旧「先頭 16 桁」正規化がこの長さ
  バイトを prefix の一部として誤って取り込んでいた。対応: `canon_ml_prefix`
  で 8B 素形（16 桁）と長さ前置形（18 桁・先頭 `40`）の両方を正しく 16 桁
  へ正規化する。
- route-table の 1 行が `ExtAddress: 0` を返し、`ext:0000000000000000` という
  ゴミ頂点が生成された。対応: `ExtAddress` が 0 の行はノード化・エッジ化
  どちらからも除外する。
