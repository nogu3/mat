...
- 各 tick で `conn` の Mutex を `try_lock` する:
  ...
  - **取れて ws がある** → 短い timeout（例 100ms）で `ws.next()` を poll する。tokio-tungstenite は
    poll 中に受けた Ping へ自動で Pong を返す。読めたフレームの扱い:
    - `Ping`/`Pong`/`Frame` → 無視（Pong 送出が目的）。
...
- **送信失敗**（`ws send failed`）: ソケットは送信前から死んでいた。ws を張り直し
  （`connect_with_retry`、短い期限 例 3s。子は温存）、**1 回だけ透過リトライ**する。
  リトライも失敗したら `teardown` してエラーを返す。
...
- コード変更なし: `--idle-timeout` の既定 300s は維持。reaper の仕組みも不変。
...
1. **keepalive が PONG を返す**: fake が接続後に Ping を送り、一定時間内に Pong を受けたら
   通過。`KEEPALIVE_INTERVAL` はテストから短縮できるようにする（`ChipToolBackend` の
   コンストラクタ引数 or `#[cfg(test)]` 用 setter。env var は避ける）。
