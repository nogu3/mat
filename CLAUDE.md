...
- `mat` is the sole owner/writer of the persistent Matter KVS (chip-tool-
  ...
  only after every listed node holds both epochs). A running `matd` picks the new IPK
  up via the `reload` admin op (`matd reload`; rotate-ipk sends it on commit and
  reports `matd_reload`) — no restart.
