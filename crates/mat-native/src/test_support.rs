...
/// interface for multicast-loopback tests: `lo` lacks `IFF_MULTICAST`
/// on Linux and never delivers. `crate::iface_select::scan()` から導出
/// （mat-controller 側の同種 2 本は別レーンで統合）。Shared here for
/// both mat-native's own group tests and mat / matd's group-routing
/// tests.
...
mod gate_tests {
    ...
    async fn fail_at_applies_to_invoke_for_data() {
        ...
        let err = conn
            .invoke_for_data(0, 1, 2, None, false)
            .await
            .unwrap_err(); // sent 1
