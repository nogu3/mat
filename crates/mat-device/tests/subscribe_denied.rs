...
//! whether the rejection leaves the session's *existing* subscription alive
//! depends on `KeepSubscriptions`, exactly as chip decides it: `true` keeps
//! it, `false` tears it down first (before path validation even runs).
//!
//! Scenario (both tests): after commissioning, demote the admin's automatic
//! ACL entry to Operate (same move as `acl_enforce.rs`), subscribe to OnOff
...
//! — rejected). The `KeepSubscriptions=true` test then turns the light on
//! and expects the *first* subscription's report, proving the rejected
//! request did not tear it down. The `KeepSubscriptions=false` test expects
//! no report at all within a window longer than the negotiated
...
/// expands to nothing readable and the request is refused with
/// `INVALID_ACTION` (same layout as `im::encode_subscribe_request`: `{0:
/// KeepSubscriptions, 1: MinIntervalFloor, 2: MaxIntervalCeiling, 3:
/// AttributeRequests [AttributePathIB{3: cluster, 4: attribute}], 7:
/// IsFabricFiltered, 255: rev}`). `SecureSession::subscribe_wildcard` can
/// only express cluster-only paths, hence the hand roll.
fn unreadable_subscribe_request(keep_subscriptions: bool) -> Vec<u8> {
    let mut w = Writer::new();
    w.start_struct(Tag::Anonymous);
...
async fn unreadable_wildcard_subscribe_is_invalid_action_and_keeps_the_existing_subscription() {
    ...
    //    KeepSubscriptions=true, so the refusal must leave the OnOff
    //    subscription from step 1 alone.
    let req = unreadable_subscribe_request(true);
...
/// up-front refusal (`INVALID_ACTION`, no readable path) must *not* leave
/// the OnOff subscription alone this time — chip tears down the
/// subscriber's existing subscriptions before path validation runs, so a
/// refused request with `KeepSubscriptions=false` still discards them.
/// Proven by turning the light on afterward and finding no report at all —
/// not even a keep-alive — within a window longer than the negotiated
/// 5s MaxInterval.
#[tokio::test]
async fn unreadable_wildcard_subscribe_with_keep_subscriptions_false_tears_down_the_existing_subscription(
    ...
    // 2. Same unreadable wildcard path as the KeepSubscriptions=true test,
    //    but KeepSubscriptions=false this time: the refusal must tear down
    //    the OnOff subscription from step 1, not leave it alone.
    let req = unreadable_subscribe_request(false);
    let exchange_id = SecureSession::new_exchange_id();
    let piggybacked = session
        .send_reliable(
            exchange_id,
    ...
    // 3. The first subscription did NOT survive: turning the light on
    //    produces no change report, and no keep-alive arrives either,
    //    within a window longer than the negotiated 5s MaxInterval —
    //    proof the subscription was torn down, not merely quiet.
    session
        .invoke(
            support::BRIDGED_EP,
            im::CLUSTER_ON_OFF,
        ...
        .expect("On invoke should succeed");
    let result = session
        .next_subscription_report(Duration::from_secs(7), &cfg)
        .await;
    assert!(
        result.is_err(),
