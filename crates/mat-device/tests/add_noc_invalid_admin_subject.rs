...
mod support;
use support::{add_noc, case_and_complete, device_config, pase_until_add_noc, spawn_device};
...
async fn add_noc_with_cat_version_zero_is_rejected_then_retry_succeeds() {
    ...
    let (mut pase, noc_tlv) = pase_until_add_noc(dev.addr, &dev.paa_der, &fabric).await;
    ...
    let mut session = case_and_complete(dev.addr, &creds).await;
