...
stage2_main() {
  ...
  cross build --release --target "$TARGET" --features ble -p matterctl -p matd
  ...
  EXPECTED_VERSION=$(cargo pkgid -p matterctl | sed 's/.*#//; s/.*@//')
...
stage1_main() {
  ...
  cargo build --release --target "$TARGET" -p matterctl -p matd
  local MAT_BIN="target/$TARGET/release/mat"
  local MATD_BIN="target/$TARGET/release/matd"
